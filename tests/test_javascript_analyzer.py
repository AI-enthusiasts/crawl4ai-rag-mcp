...
class TestJavaScriptAnalyzer:
    ...
    def test_initialization(self):
        ...
        assert self.analyzer.supported_extensions == [
            ".js",
            ".jsx",
            ".ts",
            ".tsx",
...
"""
    ...
    async def test_analyze_file_with_content(self):
        ...
        result = await self.analyzer.analyze_file(
            "/test/hello.js", "/test", content=js_content
        )
...
"""
        ...
        react_components = [
            f["name"] for f in functions if f["type"] == "react_component"
        ]
...
"""
    ...
    async def test_analyze_file_exception_handling(self):
        ...
        with patch.object(
            self.analyzer, "read_file_content", side_effect=Exception("Test error")
        ):
...
"""
    ...
    def test_parse_named_imports_with_aliases(self):
        ...
        test_cases = [
            ...
            (
                "{ Component as Comp, useState, useEffect as Effect }",
                ["Comp", "useState", "Effect"],
            ),
            (
                "{Component as Comp,useState as State}",
                ["Comp", "State"],
    ...
    def test_parse_named_imports_edge_cases(self):
        ...
        test_cases = [
            ...
            (
                "{ as Component }",
                ["Component"],
            ),  # Wrong order (should handle gracefully)
            (
                "{ Component as Comp as Extra }",
                ["Extra"],
            ),  # Multiple 'as' (take last part as fallback)
        ]
...
"""
        ...
        assert [
            "Comp",
            "State",
        ] in react_imports  # { Component as Comp, useState as State }
        assert ["Comp", "useState", "Effect"] in react_imports  # Mixed aliases
        assert [
            "DefaultExport",
            "Comp",
        ] in react_imports  # Mixed default + named with alias
