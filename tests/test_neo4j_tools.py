    ...
    async with crawl4ai_lifespan(mcp) as lifespan_context:
        ...
        try:
            ...
            if result_data["success"]:
                print(
                    f"✅ Found {len(result_data.get('repositories', []))} repositories"
                )
        ...
        try:
            ...
            if result_data["success"]:
                ...
                print(
                    f"❌ Exploration failed: {result_data.get('error', 'Unknown error')}"
                )
        except Exception as e:
...
"""
        ...
        try:
            ...
            if result_data["success"]:
                ...
                print(
                    f"   - Import hallucinations: {len(hallucinations.get('imports', []))}"
                )
                print(
                    f"   - Method call hallucinations: {len(hallucinations.get('method_calls', []))}"
                )
                print(
                    f"   - Class instantiation hallucinations: {len(hallucinations.get('class_instantiations', []))}"
                )
                ...
                print(
                    f"❌ Hallucination check failed: {result_data.get('error', 'Unknown error')}"
                )
        except Exception as e:
