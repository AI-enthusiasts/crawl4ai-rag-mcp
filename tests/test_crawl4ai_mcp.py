...
class TestSearch:
    ...
    async def test_search_basic(self, mock_scrape, mock_get):
        ...
        mock_scrape.return_value = json.dumps(
            {"success": True, "mode": "multi_url", "summary": {"successful_urls": 1}},
        )
        ...
        mock_scrape.return_value = json.dumps(
            {
                "success": True,
                "mode": "raw_markdown",
                "results": {"https://example.com": "# Raw Markdown Content"},
            },
        )
...
class TestQueryKnowledgeGraph:
    ...
    async def test_query_repos_command(self, mock_graph_db):
        ...
        mock_records = [
            MagicMock(data={"name": "repo1"}),
            MagicMock(data={"name": "repo2"}),
        ]
...
class TestSearchCodeExamplesFixed:
    ...
    @patch.dict(os.environ, {"USE_AGENTIC_RAG": "true"})
    @patch(
        "utils.search_code_examples",
    )  # This should patch the utils function
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_normalize_url_function(self):
        ...
        assert (
            normalize_url("https://example.com/page?param=1")
