...
class TestQdrantIntegrationComprehensive:
    ...
    async def _cleanup_test_data(self, adapter: QdrantAdapter):
        ...
        try:
            ...
            for i in range(100):
                try:
                    await adapter.delete_documents_by_url(
                        f"https://test.example.com/page{i}",
                    )
                    await adapter.delete_documents_by_url(
                        f"https://batch.test.com/doc{i}",
                    )
                    await adapter.delete_documents_by_url(
                        f"https://concurrent.test.com/w{i}",
                    )
                except Exception:
                    pass
    ...
    async def test_store_and_search_single_document(self, qdrant_adapter):
        ...
        with patch(
            ...
            await add_documents_to_database(
                ...
                chunk_numbers=[1],
                contents=[
                    "This is a test document about Python async programming and web crawling.",
                ],
                metadatas=[{"title": "Test Document", "author": "Test Author"}],
                url_to_full_document={
                    "https://test.example.com/doc1": "Full document content here",
                },
                batch_size=10,
            )
    ...
    async def test_store_multiple_chunks_same_url(self, qdrant_adapter):
        ...
        with patch(
            ...
            await add_documents_to_database(
                ...
                url_to_full_document={
                    "https://test.example.com/long-doc": "Complete document content",
                },
                batch_size=10,
            )
        ...
        url_results = await adapter.get_documents_by_url(
            "https://test.example.com/long-doc",
        )
    ...
    async def test_batch_processing_large_dataset(self, qdrant_adapter):
        ...
        contents = [
            f"Document {i} contains information about topic {i % 5}. This is detailed content for testing batch operations."
            for i in range(num_docs)
        ]
        metadatas = [
            {"doc_id": i, "topic": f"topic_{i % 5}", "batch": "test"}
            for i in range(num_docs)
        ]
        ...
        assert len(results) >= num_docs // 2, (
            f"Expected at least {num_docs // 2} results, got {len(results)}"
        )
    ...
    async def test_concurrent_operations(self, qdrant_adapter):
            ...
            for i in range(3):
                ...
                with patch(
                    ...
                    await add_documents_to_database(
                        ...
                        chunk_numbers=[1],
                        contents=[
                            f"Worker {worker_id} document {i} with concurrent test content",
                        ],
                        metadatas=[
                            {"worker": worker_id, "doc": i, "test": "concurrent"},
                        ],
                        url_to_full_document={
                            f"https://concurrent.test.com/w{worker_id}/d{i}": f"Full content {worker_id}-{i}",
                        },
                        batch_size=10,
                    )
        ...
        assert len(results) >= 6, (
            f"Expected at least 6 results from concurrent operations, got {len(results)}"
        )
    ...
    async def test_error_handling_embedding_failure(self, qdrant_adapter):
        ...
        with patch(
            "utils.create_embeddings_batch",
            side_effect=Exception("OpenAI API failure"),
        ):
            with pytest.raises(Exception, match="OpenAI API failure"):
                await add_documents_to_database(
                    ...
                    url_to_full_document={
                        "https://error.test.com/doc1": "Full content",
                    },
                    batch_size=10,
                )
    ...
    async def test_performance_benchmarks(self, qdrant_adapter):
        ...
        with patch(
            ...
            await add_documents_to_database(
                ...
                url_to_full_document={
                    f"https://bench.test.com/doc{i}": f"Full {i}"
                    for i in range(num_docs)
                },
                batch_size=5,
            )
